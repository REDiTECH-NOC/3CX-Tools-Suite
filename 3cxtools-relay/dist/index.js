...
const collector = new collector_1.Collector(config.pbxUrl, config.pbxExtension, config.pbxPassword);
const monitor = new monitor_1.QueueMonitor(config.pbxUrl, config.pbxExtension, config.pbxPassword);
const stateManager = new state_manager_1.StateManager();
// HTTP Pushers — wallboard is required, auto-pager is optional
const wallboardPusher = new pusher_1.Pusher(config.wallboardUrl, config.apiKey);
const autoPagerPusher = config.autoPagerUrl && config.autoPagerApiKey
    ? new pusher_1.Pusher(config.autoPagerUrl, config.autoPagerApiKey)
    : null;
let running = true;
let fastTimer = null;
...
async function refreshSlowData() {
    ...
    if (now - lastAgentsFetch >= SLOW_AGENTS_INTERVAL_MS) {
        try {
            const results = await Promise.all(cachedQueues.map(q => collector.getQueueAgents(q.Id).then(agents => ({ id: q.Id, agents }))));
...
// ─── Helpers ──────────────────────────────────────────────────
function getMidnightInTimezone(tz, now) {
 ...
 */
function calleeContainsQueue(field, queueNumber) {
    if (!field)
        return false;
...
function deriveCallState(ext, activeCalls, user, allQueueNumbers) {
    ...
    for (const call of activeCalls) {
        ...
        if (callerExt === ext || calleeExt === ext) {
            ...
            if (calleeExt === ext && allQueueNumbers.has(callerExt))
                continue;
            if (callerExt === ext && allQueueNumbers.has(calleeExt))
                continue;
            if (status.includes('talking') || status.includes('connected'))
                return 'talking';
            if (status.includes('ringing') || status.includes('routing'))
                return 'ringing';
        }
        if (call.Segments) {
            for (const seg of call.Segments) {
                ...
                if (matchesExt) {
                    ...
                    if (segStatus.includes('talking') || segStatus.includes('connected'))
                        return 'talking';
                    if (segStatus.includes('ringing') || segStatus.includes('routing'))
                        return 'ringing';
                }
            }
        }
 ...
 */
function computeQueuedCalls(queueNumber, activeCalls, serverNow, allAgentExts) {
    const queuedCalls = [];
    let longestWaitSec = 0;
    const matchesQueue = (field) => field != null && calleeContainsQueue(field, queueNumber);
    for (const call of activeCalls) {
        ...
        // Method 2b: Any segment field references queue
            ...
            const hasQueueRef = call.Segments.some(seg => [seg.CallerNumber, seg.CalleeNumber, seg.DialedDn, seg.Dn]
...
async function fastPoll() {
    ...
    try {
        ...
        const allAgentExts = new Set();
        for (const exts of queueAgentCache.values()) {
            for (const ext of exts)
                allAgentExts.add(ext);
        }
        for (const queue of cachedQueues) {
            ...
            const { queuedCalls, callsWaiting, longestWaitSec } = computeQueuedCalls(queue.Number, activeCalls, serverNow, allAgentExts);
            relayQueues.push({
                id: queue.Id,
                number: queue.Number,
                name: queue.Name,
...
async function start() {
    ...
    if (config.autoPagerUrl)
        console.log(`Auto-Pager: ${config.autoPagerUrl}/api/relay/push`);
    ...
    try {
        ...
        const agentResults = await Promise.all(queues.map(q => collector.getQueueAgents(q.Id).then(agents => ({ id: q.Id, agents }))));
